	    ...
	    _allOnlinePlayers = [];
			{
				if (isPlayer _x) then
				{
					_allOnlinePlayers pushBack _x;
				};
			} forEach playableUnits;
	    if ((count _allPlayerPositions > 0) AND (count _allOnlinePlayers > 0)) then // Only add vehicles if there are players currently playing
	    {
